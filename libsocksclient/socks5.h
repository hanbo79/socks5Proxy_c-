...
public:
  ...
  int Auth(const Ipv4Address& proxy, const std::string userName="",const std::string pwd="");
  int Connect( const Ipv4Address& host, Ipv4Address& binded);
  void Disconnect();

  int Recv(void *data, size_t size);
  int SelectRecv(void *data, size_t size, int mSeconds);
...
enum { MAX_USR_LEN = 255 };

enum { MAX_PWD_LEN = 255 };
...
namespace client {
	...
	struct LoginRequest
	{
		...
		operator const char*() const { return reinterpret_cast<const char *>(buf); }
		int sendlen() const { return static_cast<int>(3 + username.size() + password.size() ); }
	};
