...
namespace Socket
{
	...
	static bool Bind(int s, sockaddr_in& addr)
	{
		s = bind(s,(sockaddr*)&addr,sizeof(addr));
	...
	static sockaddr_in IPPort2Addr(LPCSTR ip,int port )
	{
		sockaddr_in r_addr;
	...
	static bool Select( std::vector<int> &vecCheckSocks,std::vector<int> &vecReadSocks,int mSeconds)
	{
		fd_set fdread;
		timeval tv = {mSeconds/1000, (mSeconds%1000)*1000};   
		FD_ZERO(&fdread);
		for( int i=0;i<vecCheckSocks.size();i++)
			FD_SET(vecCheckSocks[i],&fdread);

		int iRet = select(vecCheckSocks.size(), &fdread, NULL, NULL, &tv);
		if( iRet == 0 )
			return false;

		for( int i=0;i<vecCheckSocks.size();i++)
		{
			if( FD_ISSET(vecCheckSocks[i],&fdread) )
				vecReadSocks.push_back(vecCheckSocks[i]);
		}
