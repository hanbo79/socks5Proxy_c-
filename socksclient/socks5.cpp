...
void ClientTcp::Auth( const Ipv4Address& proxy, const std::string username/*=""*/,const std::string password/*=""*/ )
{
	sockaddr_in addr      = {};
...
void ClientTcp::Connect( const Ipv4Address& host, Ipv4Address& binded)
{
  {
  ...
  }
  {
    ...
    switch(res.address_type)
    {
      ...
      default: throw ErrorGeneric(fmt("Unsupported address type: ") << (int)res.address_type);
    }
  }
}
...
int Request::sendlen() const
{
  ...
  switch (address_type) {
    ...
    default: throw ErrorGeneric(fmt("Unsupported address type: ") << (int)address_type);
  }
}
...
bool Response::validate() const
{
  ...
  if (version != kSocksVersion) throw ErrorGeneric(fmt("Unsupported version: ") << (int)version);
...
#define case_reply_type(err) \
  case ReplyType_ ## err: throw Error ## err("ReplyType is " # err);
    switch (reply_type) {
      ...
      default: throw ErrorGeneric(fmt("Unsuported reply type ") << (int)reply_type);
    }
#undef case_reply_type
  ...
  if (address_type != AddressType_Ipv4 &&
      ...
      address_type != AddressType_DomainName) throw ErrorGeneric(fmt("Unsuported address type ") << (int)address_type);
  return true;
}
